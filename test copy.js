...
function test1(timeout = 1000) {
    return new Promise((resolve) => {
        let url = {
            url: `http://localhost:8081/wait/10`,
        }
...
const xhshd = {
    ...
    'Accept': '*/*',
    'User-Agent': 'XinHuaShe/8.7.6 (iPhone; iOS 15.0; Scale/2.00)',
    'Accept-Encoding': '*',
}
async function test2(timeout = 1000) {
    let url = {
        url: `http://localhost:8081/wait/4`,
        headers: xhshd,
        body: ``,
        // body: JSON.stringify(xhsBody),
    }
    try {
        axios.defaults.timeout = 1000;
        var { data } = await axios.get(url.url, url.body, {
            headers: url.headers
        });
        if (data.state == 0) {
        ...
        $.message = `发送验证码失败:${err.message}`;
    }
}


async function testSubStr(timeout = 1000) {
    allMessage = '1234567891';
    len = allMessage.length / 2 + 1
    for (let i = 0; i < len - 1; i++) {
        console.log(`${$.name}`, `${i}--${allMessage.substr(2 * i, 2)}`);
    }
}
!(async () => {
    await testSubStr();
